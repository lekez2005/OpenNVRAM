...
class SfBitlineBuffer(design.design):
    ...
    def calculate_offsets(self):
        ...
        self.active_contact_positions = [0.5*self.contact_width+active_contact_x_start+i*self.contact_pitch for i in range(3)]
    ...
    def create_bottom_inverter(self):
        ...
        for x_offset in self.poly_left_positions[1:3]:
            self.add_contact(poly_contact.layer_stack,
                             offset=vector(x_offset-0.5*(poly_contact.second_layer_width-poly_contact.first_layer_width),
                                           y_offset))
        # min m1 drc
        ...
        self.add_rect_center("active", offset=vector(self.mid_x, active_mid_y), width=self.active_width, height=active_height)
    ...
    def add_top_inverter(self):
        ...
        for i in range(2):
            self.add_layout_pin(pin_names[i], "metal2", offset=vector(x_offsets[i]-0.5*self.m2_width, nmos_via.by()),
