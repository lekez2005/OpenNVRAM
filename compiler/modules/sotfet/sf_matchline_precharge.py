...
class sf_matchline_precharge(precharge_characterization, design):
    """
    ...
    def calculate_fingers(self):
        ...
        while True:
            ...
            _, active_height = self.calculate_min_area_fill(active_width, layer=ACTIVE)
            active_height = max(active_height,
                                implant_height - 2 * self.implant_enclose_active)
    ...
    def add_nwell_contact(self):
        ...
        active_height = utils.ceil(active_area / active_width)
        active_height = max(active_contact.first_layer_width, active_height)

        self.add_rect_center(ACTIVE, offset=vector(0.5 * self.width, 0),
                             width=active_width, height=active_height)

        self.add_rect(NIMP, offset=vector(0, -0.5 * self.well_contact_implant_height),
    ...
    def add_implants(self):
        ...
        self.add_rect(PIMP, offset=vector(0, 0.5 * self.well_contact_implant_height),
    ...
    def route_source_drain(self):
        ...
        for x_offset in self.drain_positions:
            self.add_rect("metal1",
                          offset=vector(x_offset - 0.5 * self.m1_width, 0), height=active_mid_y)
