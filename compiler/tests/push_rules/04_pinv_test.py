...
class PinvTest(TestBase):
    ...
    def test_1_finger_pinv(self):
        import debug
        from modules.push_rules.pinv_push import pinv_push

        debug.info(2, "Checking 1x size inverter")
        inv = pinv_push(size=1)
    ...
    def test_medium_three(self):
        ...
        debug.info(2, "Checking three-finger inverter")
        inv = pinv_push(size=3 * 2)
        self.add_body_tap_and_test(inv)

        debug.info(2, "Checking three-finger inverter")
        inv = pinv_push(size=3 * 4)
        self.add_body_tap_and_test(inv)
    ...
    def get_max_size():
        ...
        beta = parameter["beta"]
        num_fingers = pinv_push.max_tx_mults
    ...
    def test_two_instances(self):
        ...
        inv = pinv_push(size=max_size * 1.5, beta=beta)
        self.add_body_tap_and_test(inv)
    ...
    def test_three_instances(self):
        ...
        inv = pinv_push(size=max_size * 2.5, beta=beta)
        self.add_body_tap_and_test(inv)
