...
class RouterMixin(CaravelWrapper):
    ...
    def connect_sram_power(self):
        ...
        def connect_vertical(pin_, target_name):
            if pin_.uy() > self.mid_y:
                self.connect_to_top(pin_, target_name)
            else:
                self.connect_to_bottom(pin_, target_name)

        def connect_horizontal(pin_, target_name):
            if pin_.cx() > self.mid_x:
                self.connect_to_right(pin_, target_name)
            else:
                self.connect_to_left(pin_, target_name)

        # vdd wordline
        for pin_name in ["vdd_wordline"]:
            for pin in self.sram_inst.get_pins(pin_name):
                connect_vertical(pin, VDD_WORDLINE)
        # vdd write
        for i, sram_name in enumerate(self.vdd_write_sram_pins):
            ...
            for pin in self.sram_inst.get_pins(sram_name):
                connect_horizontal(pin, caravel_name)

        for pin_name in ["vdd", "gnd"]:
            grid_name = VDD if pin_name == "vdd" else GND
            for pin in self.sram_inst.get_pins(pin_name):
                if pin.width() > pin.height():
                    if rg(pin.width()) >= rg(self.sram_inst.width):
                        ...
                        connect_horizontal(pin, grid_name)
                else:
                    if rg(pin.height()) >= rg(self.sram_inst.height):
                        ...
                        connect_vertical(pin, grid_name)
            ...
            for source_pin in source_pins:
                if source_pin.by() > self.wrapper_inst.uy() - grid_padding:
                    ...
                    if end_y > source_pin.cy():
                        ...
                        end_y = max(end_y, y_offset + self.m4_width)
                    else:
                        y_offset = source_pin.cy() + 0.5 * m3m4.height
                        end_y = min(end_y, y_offset - self.m4_width)
