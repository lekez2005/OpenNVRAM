...
class CaravelWrapper(PinAssignmentsMixin, RouterMixin, CaravelEsdMixin, design):
    ...
    def create_lvs_gds(self):
        self.lvs_gds_file = os.path.join(gds_dir, f"{wrapper_name}.lvs.gds")
        self.write_to_gds(self.lvs_gds_file)

    def write_to_gds(self, gds_file=None):
        gds_file = gds_file or os.path.join(gds_dir, f"{wrapper_name}.gds")
        self.visited = False
        self.gds_file = ""
        self.gds_read()
